...
public class TransactionsService {
  ...
  public List<CostsDetailsTransactions> detailTransForUser(
    Users nameUser,
    boolean b
  ) {
    ...
    if (listT != null) {
      for (int i = 0; i < listT.size(); i++) {
        List<CostsDetailsTransactions> listC = listT
          .get(i)
          .getCostsDetailsTransactionsList();
  ...
  public double credit(Users nameUser) {
    double credit = 0;
    String credititString = "CREDIT";
    credit = recupererDebitCredit(nameUser, credititString, credit);

    return credit;
  }

  private double recupererDebitCredit(
    Users nameUser,
    String debitCreditString,
    double debitCreditDouble
  ) {
    List<Transactions> listT = nameUser.getTransactionsList();
    for (int i = 0; i < listT.size(); i++) {
      List<CostsDetailsTransactions> listC = listT
